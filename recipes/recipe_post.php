...
//         // // 💡 更新前先清除舊的食材與步驟關聯 (避免 ID 衝突或殘留)
//         // $pdo->prepare("DELETE FROM recipe_ingredients WHERE recipe_id = ?")->execute([$target_recipe_id]);
        
//         // // 刪除步驟前要先處理步驟食材關聯
//         // $pdo->prepare("DELETE FROM step_ingredients WHERE step_id IN (SELECT step_id FROM steps WHERE recipe_id = ?)")->execute([$target_recipe_id]);
//         // $pdo->prepare("DELETE FROM steps WHERE recipe_id = ?")->execute([$target_recipe_id]);
//         // 💡 因為有了 CASCADE，我們只需刪除這兩樣，底下的 notes, tags 會自動消失
//         $pdo->prepare("DELETE FROM recipe_ingredients WHERE recipe_id = ?")->execute([$target_recipe_id]);
//         $pdo->prepare("DELETE FROM steps WHERE recipe_id = ?")->execute([$target_recipe_id]);
//     }else{
//         // --- B. 建立模式 (INSERT) ---
